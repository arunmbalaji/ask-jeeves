    ...
    return (
      <View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <View key={item.id} style={styles.container}><Text>{item.attributes.instanceType} - {item.attributes.location}</Text>
                      <Text>{item.attributes.vcpu} cores - {item.attributes.memory}</Text>
                      <Text>OnDemand Price - $ {item.ondemand.hourlyrate}</Text>
                      <Text>Operating System - {item.attributes.operatingSystem}</Text>
                      </View>}
        />
      </View>
    );
